...
public class RentACatImpl implements RentACat {
	...
	public boolean rentCat(int id) {
		...
		// If cat exists and is not rented
		// then [1] rent this cat; [2] can rented succesfully
		if ((c != null) && (!c.getRented())) {
			c.rentCat();
			return true;
		}
	...
	public String listCats() {
		...
		for (Cat c : cats) {
			// ignore all rented cats
			if (!c.getRented()) {

				// if not rented, add new line with cat to the list
				list += c.toString() + "\n";
			}
		}
